...
class LlavaLlamaModel(LlamaModel):
    ...
    def __init__(self, config: LlamaConfig, mm_vision_tower=None, mm_hidden_size=None):
        ...
        if hasattr(config, "use_mm_proj"):
            self.mm_projector = nn.Linear(config.mm_hidden_size, config.hidden_size) # 1024 4096
    ...
    def initialize_vision_modules_from_openai_clip(self, vision_tower, mm_vision_select_layer,
        ...
        if not hasattr(self, 'mm_projector'): # double check for creating mm_projector
    ...
    def extract_visual_features(self, vision_tower, images):
        ...
        if "BiomedCLIP" in self.vision_tower_name  or "biomed_clip" in self.vision_tower_name:
            ...
            select_hidden_state = image_forward_outs.hidden_states[select_hidden_state_layer] # 选择倒数第2层的feature SimCLR
    ...
    def forward(
        self,
        input_ids: torch.LongTensor = None, # [2,726]
        attention_mask: Optional[torch.Tensor] = None, # [2,726]
        ...
        orig_embeds_params = getattr(self, 'orig_embeds_params', None) # 32003, 4096
        ...
        if inputs_embeds is None:
            inputs_embeds = self.embed_tokens(input_ids) # Embedding(32003, 4096, padding_idx=0)

        vision_tower = getattr(self, 'vision_tower', None) # clip model
        if vision_tower is not None and (input_ids.shape[1] != 1 or self.training) and images is not None:
            ...
            for cur_input_ids, cur_input_embeds in zip(input_ids, inputs_embeds):
                ...
                if vision_tower.config.use_im_start_end:
                    cur_image_features = image_features[cur_image_idx] # case
                    num_patches = cur_image_features.shape[0] # 576
                    ...
                    for image_start_token_pos in image_start_tokens:
                        ...
                        if orig_embeds_params is not None: # projection
                    ...
                    new_input_embeds.append(cur_new_input_embeds)
            inputs_embeds = torch.stack(new_input_embeds, dim=0) # 2, 726, 4096

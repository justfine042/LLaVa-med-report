...
class MIMIC_CXR_Report_Dataset(data.Dataset):
    def __init__(self, data_path, split="train", transform=None, data_pct=1.0,
        ...
        self.df = self.df[self.df["ViewPosition"].isin(["PA", "AP"])] # 再筛？
        self.df["Path"] = self.df["Path"].apply(
            lambda x: os.path.join(data_path, "/".join(x.split("/")[1:])))

        # load studies and study to text mapping
        self.filenames, self.path2sent = self.load_text_data(split) #  image_path---sentence

        self.df = self.df[self.df["split"] == split]
        if data_pct != 1.0 and split == "train":
            self.df = self.df.sample(frac=data_pct, random_state=42) # 采样子集
        self.df.reset_index(drop=True, inplace=True) # sample之后reset indexs
        ...
        self.mlm_collator = DataCollatorForWholeWordMask(tokenizer=self.tokenizer, mlm=True,
                                                         mlm_probability=0.15) # 掩码整个词，而不是单个字符或子词。

        print("report sample number:", len(self.filenames)) # 227327
    ...
    def get_caption(self, path):
        series_sents = self.path2sent[path] # sentences len:6
        ...
        sent = " ".join(series_sents) # use ' ' concat all the sents above
        ...
        tokens = self.tokenizer(
            ...
            truncation=True,
            padding="max_length",
            max_length=self.max_words, # 112
        ) # pad到112的长度 使用Idx  0 pad
        x_len = len([t for t in tokens["input_ids"][0] if t != 0])

        return tokens, x_len

    def __getitem__(self, index):
        key = self.filenames[index] # image path
        caps, cap_len = self.get_caption(key) # 使用0-index pad到112  cap_len 有效文本长度 token_type_ids判断是否是同一个句子
        text, attention_mask = caps["input_ids"], caps["attention_mask"]
        caps_mask = self.mlm_collator(tuple(text)) # mask原始token并用103代替,0的地方不mask, mask的比例锁死为103
        # caps_mask['input_ids']:正常ids,mask_ids:103 0  caps_mask['labels']:只有被mask地方为原始ids, 其余全为-100
